const showTime = () => {
    ...
    document.getElementById(
        'date'
    ).innerText = `${dayText} ${date} ${monthText}`;
    document.getElementById(
        'time'
    ).innerText = `${hour}:${minutes}:${seconds} ${am_pm}`;
};
...
document
    .querySelector('.container__welcome__login')
    .addEventListener('keypress', function enterPress(e) {
        if (e.which == 13) {
            welcomeScreen();
        }
    });
...
const focusCheck = () => {
    youtubePop.addEventListener('click', () => {
        ref.innerText = 'Safari';
    });
    messageBlock.addEventListener('click', () => {
        ref.innerText = 'Messenger';
    });
    calcBody.addEventListener('click', () => {
        ref.innerText = 'Calculator';
    });
    spotlight.addEventListener('click', () => {
        ref.innerText = 'spotlight';
    });
    notesWindow.addEventListener('click', () => {
        ref.innerText = 'Notes';
    });
    gallery.addEventListener('click', () => {
        ref.innerText = 'Gallery';
    });
...
const sendThis = () => {
    ...
    switch (true) {
        ...
        case newElement.innerText.toLowerCase() == randomCode:
            ...
            setTimeout(function () {
                messageLog.append(replyElement);
                notificationSound.play();
                key1.src = './icons/unlock.svg';
        ...
        case newElement.innerText.toLowerCase() == 'cogito, ergo sum' ||
            newElement.innerText.toLowerCase() == 'cogito ergo sum':
        ...
        case newElement.innerText.toLowerCase() == "it's me":
            ...
            setTimeout(function () {
                messageLog.append(replyElement);
                marioBros.play();
                key3.src = './icons/unlock.svg';
                key3.style.opacity = '1';
            }, Math.random() * 3000);
        ...
        case newElement.innerText == '1987':
            ...
            setTimeout(function () {
                messageLog.append(replyElement);
                nokia.play();
                key4.src = './icons/unlock.svg';
                key4.style.opacity = '1';
            }, Math.random() * 3000);
            checkAnswerCondition();
        ...
        case randomReply > 6:
            replyElement.innerText =
                'Not working. Look for anything out of place.';
            break;
        case randomReply > 4:
...
const checkAnswerCondition = () => {
    if (answerArray.filter((value) => value == true).length == 5) {
        ...
        document.getElementById('youtubeVideoStream').src =
            'https://www.youtube.com/embed/-9jttTSSDzc';
        document.getElementById('videoHeading').innerText =
...
const dragElement = (elem) => {
    ...
    if (document.getElementById(elem.id + 'headersub')) {
        document.getElementById(elem.id + 'headersub').onmousedown =
            dragMouseDown;
    }
};
...
const expandYT = () => {
    if (
        youtubePop.classList.contains(
            'container__monitor__popup__youtube--fullscreen'
        )
    ) {
        youtubePop.classList.remove(
            'container__monitor__popup__youtube--fullscreen'
        );
        contentYoutube.style.height = '400px';
        youtubePop.classList.add('drag');
    } else {
        youtubePop.classList.add(
            'container__monitor__popup__youtube--fullscreen'
        );
...
const expandM = () => {
    if (
        messageBlock.classList.contains(
            'container__monitor__popup__messenger--fullscreen'
        )
    ) {
...
const expandNotes = () => {
    if (
        notesWindow.classList.contains(
            'container__monitor__popup__notes--fullscreen'
        )
    ) {
...
const expandGallery = () => {
    if (
        gallery.classList.contains(
            'container__monitor__popup__photos--fullscreen'
        )
    ) {
...
const menuControl = (valueID) => {
    let register = 0;
    if (
        document.querySelector(`.container__monitor__nav__dropdown--${valueID}`)
            .style.visibility == 'visible'
    ) {
        document
            .getElementById(valueID)
            .classList.remove('container__monitor__nav__left__icon--active');
        register = 1;
    }
    ...
    if (register != 1) {
        document.querySelector(
            `.container__monitor__nav__dropdown--${valueID}`
        ).style.visibility = 'visible';
        for (value of tableArray) {
            document
                .getElementById(value)
                .classList.remove(
                    'container__monitor__nav__left__icon--active'
                );
        }
        document
            .getElementById(valueID)
            .classList.add('container__monitor__nav__left__icon--active');
    }
};

const contentYoutube = document.querySelector(
    '.container__monitor__popup__youtube__content'
);
const aboutThis = document.querySelector(
    '.container__monitor__nav__left__icon--OS'
);
...
const sendMessage = document.querySelector(
    '.container__monitor__popup__messenger__text__textInput__img'
);
const messageConnect = document.getElementById('msg');
const messageLog = document.querySelector(
    '.container__monitor__popup__messenger__text__messages'
);
const msngr = document.getElementById('msngr');
const messageWindow = document.querySelector(
    '.container__monitor__popup__messenger'
);
const calcClose = document.querySelector('.calc-body__topBar__button--close');
const messageClose = document.querySelector(
    '.container__monitor__popup__messenger__contacts__topBar__button--close'
);
const messageBlock = document.getElementById('messageBlock');
const fullScreenYT = document.querySelector(
    '.container__monitor__popup__youtube__web__controls__button--quack'
);
const randomCode = hiddenMessage();
const fullScreenM = document.querySelector(
    '.container__monitor__popup__messenger__contacts__topBar__button--quack'
);
const monitor = document.querySelector('.container__monitor__popup');
const ref = document.getElementById('check');

const notesExpand = document.querySelector(
    '.container__monitor__popup__notes__controls__button--quack'
);
const closeNotes = document.querySelector(
    '.container__monitor__popup__notes__controls__button--close'
);
const notesWindow = document.querySelector('.container__monitor__popup__notes');
const notes = document.getElementById('notes');
const spotlight = document.querySelector(
    '.container__monitor__popup__spotlight'
);
...
const galleryClose = document.querySelector(
    '.container__monitor__popup__photos__controls__button--close'
);
const galleryExpand = document.querySelector(
    '.container__monitor__popup__photos__controls__button--quack'
);

//Gallery
const image1 = document.querySelector(
    '.container__monitor__popup__photos__gallery__preview--1'
);
const image2 = document.querySelector(
    '.container__monitor__popup__photos__gallery__preview--2'
);
const image3 = document.querySelector(
    '.container__monitor__popup__photos__gallery__preview--3'
);
const image4 = document.querySelector(
    '.container__monitor__popup__photos__gallery__preview--4'
);
...
const changeImage = (image) => {
    ...
    console.log(imageTitle);
    document.querySelector(
        '.container__monitor__popup__photos__controls__title'
    ).innerText = imageTitle;
};
...
//Drop Downs
const duckDropDown = document.querySelector(
    '.container__monitor__nav__dropdown--duck'
);
const mainDropDown = document.querySelector(
    '.container__monitor__nav__dropdown--main'
);
const fileDropDown = document.querySelector(
    '.container__monitor__nav__dropdown--file'
);
const editDropDown = document.querySelector(
    '.container__monitor__nav__dropdown--edit'
);
const viewDropDown = document.querySelector(
    '.container__monitor__nav__dropdown--view'
);
const helpDropDown = document.querySelector(
    '.container__monitor__nav__dropdown--help'
);
...
dateClick.addEventListener('click', () => {
    minimise(calendarApp);
});
timeClick.addEventListener('click', () => {
    minimise(calendarApp);
});
...
const calendarFill = (day, date) => {
    ...
    for (let i = 0; i < monthDays; i++) {
        document.querySelector(
            `.calendar__day__row__col--${dayConversion + 1 + i}`
        ).innerText = i + 1;
        if (i + 1 == date) {
            document
                .querySelector(
                ...
                .classList.add('calendar__day__row__col--today');
        }
    }
    for (let i = 1; i < 37; i++) {
        if (i % 7 == 6 || i % 7 == 0) {
            if (
                document
                    .querySelector(`.calendar__day__row__col--${i}`)
                    .classList.contains('calendar__day__row__col--today')
            ) {
            } else {
                document.querySelector(
                    `.calendar__day__row__col--${i}`
                ).style.color = 'gray';
            }
        }
    }
