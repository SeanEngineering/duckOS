...
const expandGallery = () => {
  if (
    gallery.classList.contains('container__monitor__popup__photos--fullscreen')
  ) {
...
const calendarFill = (day, date) => {
  ...
  for (let i = 0; i < monthDays; i++) {
    ...
    if (i + 1 == date) {
      document
        .querySelector(`.calendar__day__row__col--${dayConversion + 1 + i}`)
  ...
  for (let i = 1; i < 37; i++) {
    if (i % 7 == 6 || i % 7 == 0) {
      if (
        ...
        document.querySelector(`.calendar__day__row__col--${i}`).style.color =
          'gray';
      }
    }
  }
